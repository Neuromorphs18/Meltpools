...
def ImportAedatDataVersion1or2(aedat):
    ...
    if info['source'] == 'Das1':
        ...
        # To do: Dvs128
        specialMask = int('8000',16)
        #Assuming these are all 1's and zeros
        specialLogical = np.bitwise_and(allAddr,specialMask)
        specialLogical = specialLogical.astype(bool)
        #specialLogical = np.logical_and(allAddr,specialMask)
        polarityLogical = np.logical_not(specialLogical)
        if ('dataTypes' not in importParams or 'special' in importParams['dataTypes'])  and any(specialLogical):
            outputData['special'] = {}
            outputData['special']['timeStamp'] = allTs[specialLogical]
        del specialLogical
        # Polarity(DVS) events
        if ('dataTypes' not in importParams or 'polarity' in importParams['dataTypes']) and any(polarityLogical):
            ...
            yMask = int('7F00',16)
            yShiftBits = 8
            outputData['polarity']['y'] = np.array(np.right_shift(np.bitwise_and(polarityData, yMask), yShiftBits), 'uint16')
            # X addresses
            xMask = int('fE',16)
            xShiftBits = 1
            outputData['polarity']['x'] = np.array(np.right_shift(np.bitwise_and(polarityData, xMask), xShiftBits), 'uint16')
            # Polarity bit

            # Note: no need for a bitshift here, since its converted to boolean anyway
            polarityMask = int('0001',16)
            outputData['polarity']['polarity'] = np.array(np.bitwise_and(polarityData, polarityMask), 'bool')
            del polarityData
        del polarityLogical
